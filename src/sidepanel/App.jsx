...
export default function App() {
  ...
  useEffect(() => {
    ...
    getProviders().then((allProviders) => {
      ...
      getSettings().then((s) => {
        const defaultProv =
          allProviders.find((p) => p.id === s.defaultProviderId) ||
          allProviders[0];
      ...
      if (area === "sync" && changes.providers) {
        getProviders().then((allProviders) => {
          ...
          setActiveProviderId((prev) =>
            allProviders.some((p) => p.id === prev)
              ? prev
              : allProviders[0]?.id || null,
          );
        });
      }
          ...
          <div className="chat-messages">
            {messages.length === 0 && !isStreaming && (
              <div className="empty-state">
                Select text on a page and choose a prompt to get started.
              </div>
            )}
            {messages.map((msg, i) => (
              <ChatMessage
